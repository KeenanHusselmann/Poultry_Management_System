...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.egg-productions.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="eggs_produced">{{ trans('cruds.eggProduction.fields.eggs_produced') }}</label>
                <input class="form-control {{ $errors->has('eggs_produced') ? 'is-invalid' : '' }}" type="number" name="eggs_produced" id="eggs_produced" value="{{ old('eggs_produced', '') }}" step="1">
