...
@can('create_chemical_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.create-chemicals.create') }}">
                        ...
                        <tr data-entry-id="{{ $createChemical->id }}">
                            ...
                            <td>
                                @can('create_chemical_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.create-chemicals.show', $createChemical->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('create_chemical_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.create-chemicals.edit', $createChemical->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('create_chemical_delete')
                                    <form action="{{ route('admin.create-chemicals.destroy', $createChemical->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('create_chemical_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.create-chemicals.massDestroy') }}",
