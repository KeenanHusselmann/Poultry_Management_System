...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.lifeStock.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.life-stocks.update", [$lifeStock->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('breed') ? 'is-invalid' : '' }}" name="breed" id="breed" required>
                    ...
                    @foreach(App\Models\LifeStock::BREED_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('breed', $lifeStock->breed) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('breed'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('purpose') ? 'is-invalid' : '' }}" name="purpose" id="purpose" required>
                    ...
                    @foreach(App\Models\LifeStock::PURPOSE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('purpose', $lifeStock->purpose) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_of_birth') ? 'is-invalid' : '' }}" type="text" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $lifeStock->date_of_birth) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('weight') ? 'is-invalid' : '' }}" type="number" name="weight" id="weight" value="{{ old('weight', $lifeStock->weight) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('purchase_date') ? 'is-invalid' : '' }}" type="text" name="purchase_date" id="purchase_date" value="{{ old('purchase_date', $lifeStock->purchase_date) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('notes') ? 'is-invalid' : '' }}" name="notes" id="notes">{{ old('notes', $lifeStock->notes) }}</textarea>
                @if($errors->has('notes'))
                    <div class="invalid-feedback">
                        {{ $errors->first('notes') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('health_status') ? 'is-invalid' : '' }}" name="health_status" id="health_status">
                    ...
                    @foreach(App\Models\LifeStock::HEALTH_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('health_status', $lifeStock->health_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('number_of_birds') ? 'is-invalid' : '' }}" type="number" name="number_of_birds" id="number_of_birds" value="{{ old('number_of_birds', $lifeStock->number_of_birds) }}" step="1" required>
