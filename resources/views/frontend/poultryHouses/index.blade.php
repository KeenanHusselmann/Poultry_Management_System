        ...
        <div class="col-md-12">
            @can('poultry_house_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.poultry-houses.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $poultryHouse->id }}">
                                        ...
                                        <td>
                                            @can('poultry_house_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.poultry-houses.show', $poultryHouse->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('poultry_house_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.poultry-houses.edit', $poultryHouse->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('poultry_house_delete')
                                                <form action="{{ route('frontend.poultry-houses.destroy', $poultryHouse->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('poultry_house_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.poultry-houses.massDestroy') }}",
