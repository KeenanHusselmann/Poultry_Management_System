    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.health-records.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="live_stock_id" id="live_stock_id" required>
                                @foreach($live_stocks as $id => $entry)
                                    <option value="{{ $id }}" {{ old('live_stock_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="disease_id" id="disease_id">
                                @foreach($diseases as $id => $entry)
                                    <option value="{{ $id }}" {{ old('disease_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control date" type="text" name="record_date" id="record_date" value="{{ old('record_date') }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="desc_of_sick" id="desc_of_sick">{{ old('desc_of_sick') }}</textarea>
