        ...
        <div class="col-md-12">
            @can('egg_production_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.egg-productions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $eggProduction->id }}">
                                        ...
                                        <td>
                                            @can('egg_production_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.egg-productions.show', $eggProduction->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('egg_production_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.egg-productions.edit', $eggProduction->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('egg_production_delete')
                                                <form action="{{ route('frontend.egg-productions.destroy', $eggProduction->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('egg_production_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.egg-productions.massDestroy') }}",
