        ...
        <div class="col-md-12">
            @can('disease_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.diseases.create') }}">
                            ...
                            <tbody>
                                @foreach($diseases as $key => $disease)
                                    <tr data-entry-id="{{ $disease->id }}">
                                        ...
                                        <td>
                                            @can('disease_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.diseases.show', $disease->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('disease_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.diseases.edit', $disease->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('disease_delete')
                                                <form action="{{ route('frontend.diseases.destroy', $disease->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('disease_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.diseases.massDestroy') }}",
