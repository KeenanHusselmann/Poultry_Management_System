        ...
        <div class="col-md-12">
            @can('pesticide_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.pesticides.create') }}">
                            ...
                            <tbody>
                                @foreach($pesticides as $key => $pesticide)
                                    <tr data-entry-id="{{ $pesticide->id }}">
                                        ...
                                        <td>
                                            @can('pesticide_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.pesticides.show', $pesticide->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('pesticide_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.pesticides.edit', $pesticide->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('pesticide_delete')
                                                <form action="{{ route('frontend.pesticides.destroy', $pesticide->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pesticide_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.pesticides.massDestroy') }}",
