        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.feedManagment.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.feed-managments.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="livestock_id" id="livestock_id">
                                @foreach($livestocks as $id => $entry)
                                    <option value="{{ $id }}" {{ old('livestock_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control datetime" type="text" name="feeding_date" id="feeding_date" value="{{ old('feeding_date') }}" required>
