        ...
        <div class="col-md-12">
            @can('feed_managment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.feed-managments.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $feedManagment->id }}">
                                        ...
                                        <td>
                                            @can('feed_managment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.feed-managments.show', $feedManagment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('feed_managment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.feed-managments.edit', $feedManagment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('feed_managment_delete')
                                                <form action="{{ route('frontend.feed-managments.destroy', $feedManagment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('feed_managment_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.feed-managments.massDestroy') }}",
