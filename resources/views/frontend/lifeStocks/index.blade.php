        ...
        <div class="col-md-12">
            @can('life_stock_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.life-stocks.create') }}">
                            ...
                            <tbody>
                                @foreach($lifeStocks as $key => $lifeStock)
                                    <tr data-entry-id="{{ $lifeStock->id }}">
                                        ...
                                        <td>
                                            @can('life_stock_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.life-stocks.show', $lifeStock->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('life_stock_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.life-stocks.edit', $lifeStock->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('life_stock_delete')
                                                <form action="{{ route('frontend.life-stocks.destroy', $lifeStock->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('life_stock_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.life-stocks.massDestroy') }}",
